...
################################################################################################
# h5: parasite within subplots 
##########################################################################################
parasite <- river  %>% filter(river == "Passaic") %>% group_by(river, season, plot, subplot) %>%
...
# global nmds
#ordO_p<-metaMDS(pt, distance="bray", trymax = 200, autotransform=FALSE) 
ordO_p<-metaMDS(p01, distance="bray", trymax = 200, autotransform=FALSE) 
...
nmds_parasite_passaic <-ggplot(parasite_all, aes(x= NMDS1, y=NMDS2))+
...
r_para_pa<-adonis(formula=p01_pa ~ as.factor(plot), data=envi01_pa,  permutations=9999, method="bray")
r_para_pa

############################################################################
# h2. habitat variabbles within subplots  
##########################################################################################
habitat01 <- river  %>% filter(river == "Passaic")%>% group_by (river, season, plot, subplot) %>%
  summarise_all(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) %>%
...
# habitat <- semi_join(habitat01, parasite_all, by=c("river", "season", "plot", "subplot"))
...
# PERMANOVA
p_envi<-adonis(formula=hab ~ as.factor(plot), data=hab_2,  permutations=999, method="bray") 
p_envi

sitecoordO_hab<-as.data.frame(scores(ordO_hab, display="sites"))#extracts coordinates for plots
habitat_all <- bind_cols(hab_2, sitecoordO_hab)

topp<-max(habitat_all[,5:6]) 
bott<-min(habitat_all[,5:6]) 

habitat_all$plot <- as.factor(habitat_all$plot)
###########################################################################################
###########################################################################################
...
nmds_habitat_Passaic <-ggplot(habitat_chem, aes(x= PC1, y=PC2))+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
  geom_point( size =2.75, aes(fill=plot), shape=21)+
  xlab("PC 1") +
...
nmds_phy_Passaic <-ggplot(habitat_phy, aes(x= PC1, y=PC2))+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
  geom_point( size =2.75, aes(fill=plot), shape=21)+
  xlab("PC 1") +
...
################################################################################################
# h3: host community abundance within subplot 
##########################################################################################
# abundance based FISH MATRIX 
fish_01 <- river %>% filter (river == "Passaic") %>% mutate(abun = rep(1)) %>% group_by(river, plot, season, subplot, host_species) %>%
  summarise_all(funs(if(is.numeric(.)) sum(., na.rm = TRUE) else first(.))) %>% spread(host_species, abun)
fish_01[,20:88][is.na(fish_01[,20:88])] <- 0
fish <- fish_01 %>% group_by(river, plot, subplot, season) %>%
  summarise_all(funs(if(is.numeric(.)) sum(., na.rm = TRUE) else first(.))) %>%
  group_by(river, plot, season) %>%
  summarise_all(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.)))
...
fish_matrix<-fish[,27:48] # makes new matrix with fish data (columns 18-41)
...
###########################################################################################
nmds_fish_abun_passaic <-ggplot(fish_all, aes(x= NMDS1, y=NMDS2))+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
  geom_point( size =2.75, aes(fill=plot), shape=21)+
  xlab("NMDS 1") +
...
############### Jaccard ####################################################
fish_pa<-apply(fish[,27:48],2, function(x) {ifelse(x==0,0,1)})#makes presence-absence matrix
fish_01pa<-fish_pa[rowSums(abs(fish_pa))!=0,] 
fish_envi01pa<-fish_envi[rowSums(abs(fish_pa))!=0,] 

#Bray-Curtis NMDS
ordOpa<-metaMDS(fish_01pa, distance="bray", trymax = 100, autotransform=FALSE)#global NMDS
stressplot(ordOpa)
ordOpa
...
nmds_fish_pa <-ggplot(fish_all_pa, aes(x= NMDS1, y=NMDS2))+
  ...
  guides(fill=guide_legend("Plot",  keywidth=0.12,
                           ...
                           default.unit="inch"))
################################################################################################
# h4: host community biomass
##########################################################################################
fish_02 <- river %>% filter (river == "Passaic") %>% mutate(abun = rep(1)) %>% group_by(river, plot, season, subplot, host_species) %>%
  summarise_all(funs(if(is.numeric(.)) sum(., na.rm = TRUE) else first(.))) %>% spread(host_species, dry_weight_g)
fish_02[,20:88][is.na(fish_02[,20:88])] <- 0
fish2 <- fish_02 %>% group_by(river,season, plot, subplot) %>%
  summarise_all(funs(if(is.numeric(.)) sum(., na.rm = TRUE) else first(.))) %>%
  group_by(river,season, plot) %>%
  summarise_all(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
...
# abundance based matrix 
fish_matrix2<-fish2[,27:48] # makes new matrix with fish data (columns 18-41)
...
###########################################################################################
nmds_biomass_passiac <-ggplot(fish_all_02, aes(x= NMDS1, y=NMDS2))+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
  geom_point( size =2.75, aes(fill=plot), shape=21)+
  xlab("NMDS 1") +
...
ggarrange(nmds_habitat_Passaic, nmds_phy_Passaic, nmds_fish_abun_passaic, nmds_biomass_passiac, 
          nmds_parasite_passaic, common.legend = TRUE, ncol = 2, nrow = 3)
dev.off()
...
# space instead of complete dissimilarity matrices. 
...
#plot(pro_3, kind="1")
#dev.off()
prot_hb_ha <- protest(ordO_02, ordO)
prot_hb_ha
...
#jpeg(file="Passaic_procrustes_host_parasite.jpeg", width = 360, height = 180, units = 'mm', res = 300)
#par(mfrow=c(1,2))
#plot(pro_1, kind="1")
#plot(pro_1, kind="2")
#dev.off()
prot_ha_p <- protest(ordO, ordO_p)
prot_ha_p
...
#plot(pro_5, kind="1")
prot_hb_p <- protest(ordO_02, ordO_p)
prot_hb_p
...
#plot(pro_5, kind="1")
prot_env_p <- protest(ordO_hab, ordO_p)
prot_env_p
...
jpeg(file="Passaic_procrustes_errors.jpeg", width = 270, height = 270, units = 'mm', res = 300)
par(mfrow=c(3,3))
...
Passaic_protest <-data.frame(a,b,c,d,e,f,g,h,i,j,k,l)
...
colnames(Passaic_protest) <- blah
rownames(Passaic_protest) <- meh
###########################################################################################
...
passaic_permanova <-rbind(parasite_permanova, fish_numerical_permanova, fish_biomass_permanova)
...
pca_ordinations_passaic <-ggplot(ordo_all, aes(x= PC1, y=PC2))+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
...
jpeg(file="Passaic_river_ordinations.jpeg", width = 100, height = 100, units = 'mm', res = 300)
pca_ordinations_passaic
dev.off()
