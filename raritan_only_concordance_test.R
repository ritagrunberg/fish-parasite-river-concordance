...
parasite_species <- data.frame(parasite = c('Nematode sp1', 'C. margninatum', 'Eustronglyoides', 'Leptorhynchoides (la.)',
                                            'Neoechinorhynchus (la)', 'Proteocephalus', 'Bothriocephalus','Spathebothriidea' ,
                                            'Plagioporus','Strigeoidae (mc)', 'Alloglossidium', 'Isoglaridacris', 'Pilum',
                                            'Fessesentis', 
                                            'C. isostomum', 
                                            'C. cooperi',  'Acanthocephalus', 'N. cristatus', 'Triganodistomum', 'Echinorhynchus',
                                            'Camallanus', "L. thecatus", 'Rhabdochona', 'P. minimum', 'Heterophyidae (mc)', 'U. ambloplitis',
                                            'Tremtaode sp1',
                                            'Philonema', 'Tapeworm sp1',
                                            'Pomporhynchus', 'Spinitectus', 'Raphidascaris','N. cylindratus',
                                            'A. commune', 
                                            'Red nem. sp2')
                               )
...
##########################################################################################
############################################################################
habitat01 <- river  %>% filter(river == "Raritan")%>% 
  group_by (river, season, plot, subplot) %>%
  summarise_all(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) %>%
  group_by(river, plot, season) %>%
  summarise_all(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) %>%
...
fish <- fish_01 %>%
  ...
  group_by(river, plot, season) %>%  
   filter(!(river == "Raritan" & season == "summer" & plot ==1)) %>%  # remove summer plot 1 observation becaues no parasite data for that 
  summarise_all(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.)))
fish <- fish[,-c(27:65,84)] # remove parasite data
...
#p_abun<-adonis(formula=fish_01 ~ as.factor(plot) * river , data=fish_envi01,  permutations=999, method="bray")
...
fish_species <- data.frame(fish = c('C. commersonii', 'C. spiloptera', 'E. americanus', 'E. olmstedi',
                                    'F. diaphanus', 'G. holbrooki', 'L. auritus', 'L. gibbosus', 'L. macrochirus',
                                    'M. dolomieu', 'M. salmoides','N. amoenus','N. bifrenatus','N. gyrinus', 'R. atratulus',
                                    'R. cataractae',  'S. atromaculatus', 'S. corporalis'))
...
r_fishpa<-adonis(formula=fish_01pa ~ as.factor(plot) , data=fish_envi01pa,  permutations=999, method="jaccard")
r_fishpa
...
##########################################################################################
fish_02 <- river %>% filter (river == "Raritan") %>% 
  mutate(abun = rep(1)) %>% 
...
fish2 <- fish_02 %>%
  ...
  group_by(river,plot,season) %>%  
   filter(!(river == "Raritan" & season == "summer" & plot ==1)) %>%  # remove summer plot 1 observation becaues no parasite data for that 
  summarise_all(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
fish2 <- fish2[,-c(27:65, 84)]  # remove parasite data
...
jpeg(file="Raritan_River_ordinations_biomass.jpeg", width = 120, height = 120, units = 'mm', res = 300)
nmds_biomass
dev.off()
#############################################################################################
...
#plot(pro_1, kind="1")
prot_ha_envchem <- protest(ordO_hab_chem, ordO,permutations = 999)
prot_ha_envchem
i <- c(prot_ha_envchem$ss, prot_ha_envchem$signif, "fish_abundance", "chemical")

# host abundance and phys
pro_13 <- procrustes( ordO_hab_phy, ordO)
#plot(pro_1, kind="1")
prot_ha_envphy <- protest(ordO_hab_phy,ordO, permutations = 999)
prot_ha_envphy
j <- c(prot_ha_envphy$ss, prot_ha_envphy$signif, "fish_abundance", "physical")

# host biomass and chemical
pro_14 <- procrustes( ordO_hab_chem, ordO_02)
#plot(pro_1, kind="1")
prot_bio_envchem <- protest(ordO_hab_chem, ordO_02, permutations = 999)
prot_bio_envchem
k <- c(prot_bio_envchem$ss, prot_bio_envchem$signif, "fish_biomass", "chemical")

# host biomass and phys
pro_15 <- procrustes(ordO_hab_phy, ordO_02)
#plot(pro_1, kind="1")
prot_bio_envphy <- protest(ordO_hab_phy,ordO_02, permutations = 999)
prot_bio_envphy
...
pro_16$X # target ordination
pro_16$Yrot # rotated ordination
plot(pro_16)
...
jpeg(file="procrustes_error_host_parasite_pa_Raritan.jpeg", width = 90, height = 90, units = 'mm', res = 300)
 ggplot(proc_out)+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
...
write.csv(res_host_parasite_pa, "residuals_host_parasite_pa_raritan.csv")

jpeg(file="Raritan_host_parasite_PA_residuals.jpeg", width = 90, height = 90, units = 'mm', res = 300)
...
#plot(pro_1, kind="1")
prot_par_host <- protest( ordO, ordO_p_pa,permutations = 999)
prot_par_host
...
proc_out.parasite <- data.frame(NMDS1=pro_20$Yrot[,1], # parasite
                            NMDS2=pro_20$Yrot[,2], # parasite
                            xNMDS1=pro_20$X[,1], #habitat
                            xNMDS2=pro_20$X[,2]) # habitat 
pro_20$rotation

jpeg(file="procrustes_error_parasite_phys_pa_Raritan.jpeg", width = 90, height = 90, units = 'mm', res = 300)
ggplot(proc_out.parasite)+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
  geom_point(aes(x=NMDS1, y=NMDS2), fill="black", pch=21, size=1.5)+ # parasite
  geom_point(aes(x=xNMDS1, y=xNMDS2), fill="#636363", pch=21, size=1.5)+ # phys 
  geom_segment(aes(x=NMDS1,y=NMDS2,xend=xNMDS1,yend=xNMDS2),
               arrow=arrow(length=unit(0.2,"cm")), color="#1c9099", lwd=0.5)+ theme_pubr()+
  geom_segment(aes(x=pro_16$rotation[1,2],y=-1,
                   xend=-pro_16$rotation[1,2],yend=1.5), lwd=0.25)+
  geom_segment(aes(x=-1,y=-pro_16$rotation[1,2],
                   xend=1.5,yend=pro_16$rotation[1,2]), lwd=0.25)+
  labs(x="Axis 1", y = "Axis 2")+
  #  ylim(c(-0.75,0.75))+ xlim(c(-0.75,0.75))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))# add black square panel around graphic
dev.off()
...
write.csv(res_host_phys_pa, "residuals_host_phys_pa_raritan.csv")
############################################################

proc_out.host <- data.frame(NMDS1=pro_21$Yrot[,1], # fish
                       NMDS2=pro_21$Yrot[,2], #fish
                       xNMDS1=pro_21$X[,1], #habitat
                       xNMDS2=pro_21$X[,2]) # habitat 
pro_21$rotation

jpeg(file="procrustes_error_host_phys_pa_Raritan.jpeg", width = 90, height = 90, units = 'mm', res = 300)
ggplot(proc_out.host)+
  geom_hline(yintercept = 0, lty=2, color="grey") +
  geom_vline(xintercept = 0, lty=2, color="grey") +
  geom_point(aes(x=NMDS1, y=NMDS2), fill="white", pch=21, size=1.5)+ # host
  geom_point(aes(x=xNMDS1, y=xNMDS2), fill="#636363", pch=21, size=1.5)+ # phys 
  geom_segment(aes(x=NMDS1,y=NMDS2,xend=xNMDS1,yend=xNMDS2),
               arrow=arrow(length=unit(0.2,"cm")), color="#1c9099", lwd=0.5)+ theme_pubr()+
  geom_segment(aes(x=pro_16$rotation[1,2],y=-1,
...
write.csv(res_host_parasite, "residuals_hostpa_parasite_raritan.csv")
...
raritan_river_protest<-rbind(c,d,e,
                             g,h,
                             i,j,k,l, 
                             m, n, o, p, q,
                             r, s, t, u, 
                             v, 
                             w, x)
raritan_river_protest <- as.data.frame(raritan_river_protest)
raritan_river_protest <- raritan_river_protest %>% dplyr::select(1:4) 
meh <-c('ms_square', "p_value", "var1", "var2")
colnames(raritan_river_protest) <- meh
raritan_river_protest <- raritan_river_protest %>% mutate(river =rep("Raritan"))

write.csv(raritan_river_protest, "raritan_river_protest.csv")
...
Raritan_permanova <-rbind(parasite_permanova, parasite_pa_permanova, 
...
write.csv(Raritan_permanova, "raritan_river_PERMANOVA.csv")
